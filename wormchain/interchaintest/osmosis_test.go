...
func TestOsmosis(t *testing.T) {
	...
	osmosisChain := chains[0].(*cosmos.CosmosChain)
	...
	users := interchaintest.GetAndFundTestUsers(t, ctx, t.Name(), userFunds, osmosisChain)
	osmoUser := users[0]

	codeId, err := osmosisChain.StoreContract(ctx, osmoUser.KeyName, "./contracts/cw20_wrapped_2.wasm")
	require.NoError(t, err)
